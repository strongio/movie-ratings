...
ggplot(data, aes(x=metacritic_scaled, y=imdb_scaled)) +
                ...
                scale_y_continuous('Metacritic', breaks=c(-2,2), labels=c('Low','High')) +
                scale_x_continuous('IMDB', breaks=c(-2,2), labels=c('Low','High')) +
                theme_strong
...
ggplot(data %>% filter(!is.na(year) & year < 2016), aes(x=metacritic_scaled, y=imdb_scaled)) +
                geom_point(alpha=.5) +
                stat_smooth(method="lm", color='#00cbfe', se=F) +
                scale_y_continuous('Metacritic', labels=c()) +
                scale_x_continuous('IMDB', labels=c()) +
                facet_wrap(~year) +
                theme_strong
